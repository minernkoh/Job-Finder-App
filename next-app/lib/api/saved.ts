...
export async function unsaveListing(listingId: string): Promise<void> {
  ...
  if (!res.data.success) throw new Error("Failed to unsave");
}
...
export async function fetchSavedListings(): Promise<SavedListingResult[]> {
  const res = await apiClient.get<{
    success: boolean;
    data: { listings: SavedListingResult[] };
  }>("/api/v1/saved");
  if (!res.data.success) throw new Error("Failed to fetch saved");
